...
router.get("/", verifyToken, async (req, res) => {
  try {
    ...
    const [selesai, diisi, proses, belum] = await Promise.all([
      db.get2("SELECT COUNT(*) as c FROM instrumen WHERE status='selesai'"),
      db.get2("SELECT COUNT(*) as c FROM instrumen WHERE status='diisi'"),
      db.get2("SELECT COUNT(*) as c FROM instrumen WHERE status='proses'"),
      db.get2("SELECT COUNT(*) as c FROM instrumen WHERE status='belum'"),
    ]);

    const rataSkorRow = await db.get2("SELECT AVG(skor) as avg FROM hasil_audit");
    ...
    const statusDistribusi = await db.all2(`
      SELECT status, COUNT(*) as jumlah FROM instrumen GROUP BY status
    `);
